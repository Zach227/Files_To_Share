...
parser.add_argument("-md", "--modulesdirectory", help="collects the module data for all files in directory")
parser.add_argument("-f", "--file", help="collects the module data for a single file")
parser.add_argument("-pd", "--participantsdirectory", help="collects the data for each participant from a directory")
...
def get_student_info(file_name, data):
    ...
    for student in student_list:
        if student not in student_to_modules:
            student_to_modules[student] = []       

        student_data = data.iloc[i,:]
        # if student_data[6] == "A":
        student_to_modules[student].append(module_name)
        i = i+1 
...
if args.file:
    ...
    print(student_to_modules["Samuel Reynolds"])